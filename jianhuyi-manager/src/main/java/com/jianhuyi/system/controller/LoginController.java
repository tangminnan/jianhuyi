...
public class LoginController extends BaseController {
  ...
  ModelAndView main() throws ParseException {
    ...
    // 查询所有数据的人和日期
    LinkedList<UseJianhuyiLogDO> useJianhuyiLogDOLinkedList =
        useJianhuyiLogDao.selectPersonAndDate(params);
    ...
    for (DataEverydayDO useJianhuyiLogDO : everydayDOList) {
      ...
      if (useJianhuyiLogDO.getReadLight() != null) avgReadLight += useJianhuyiLogDO.getReadLight();
      if (useJianhuyiLogDO.getSitTilt() != null) avgSitTilt += useJianhuyiLogDO.getSitTilt();
      if (useJianhuyiLogDO.getEffectiveTime() != null)
        avgUseJianhuyiDuration += useJianhuyiLogDO.getEffectiveTime();
    }

    // 首页各等级数据统计
    mav.addObject("avgReadDuration", avgReadDuration / everydayDOList.size());
    mav.addObject("avgOutdoorsDuration", avgOutdoorsDuration / everydayDOList.size());
    mav.addObject("avgReadDistance", avgReadDistance / everydayDOList.size());
    mav.addObject("avgReadLight", avgReadLight / everydayDOList.size());
    mav.addObject("avgLookPhoneDuration", avgLookPhoneDuration / everydayDOList.size());
    mav.addObject("avgLookTvComputerDuration", avgLookTvComputerDuration / everydayDOList.size());
    mav.addObject("avgSitTilt", avgSitTilt / everydayDOList.size());
    mav.addObject("avgUseJianhuyiDuration", avgUseJianhuyiDuration / everydayDOList.size());

    mav.addObject("num", num);
    mav.addObject("yesterdayNum", yesterdayNum);
    mav.addObject("useJianhuyiLogDOList", JSON.toJSONString(everydayDOList));
  ...
  List<EchartsDO> getReadDuration(@RequestBody String useJianhuyiLogDOList) throws IOException {
    List<UseJianhuyiLogDO> useJianhuyiLogDOList1 =
        JSON.parseArray(useJianhuyiLogDOList, UseJianhuyiLogDO.class);
